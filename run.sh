...
#SBATCH --gpus=2
#SBATCH --mem=228G
#SBATCH --job-name=fixed-byte-sampler
#SBATCH --requeue
...
export CLEAN_MODEL_PATH="common-pile/comma-v0.1-2t"
export BATCH_SIZE=2
NUM_SHARDS=8
...
for TEMP in ${TEMPS[@]}; do
    uv run python -m generate_cd \
        ...
        --output_file /gscratch/kohlab/jyyh/fixed_bs/copyright_${SIZE}_results_toklevel_temp_${TEMP}_k_radius_${K_RADIUS}_shard${size_idx}.jsonl \
    ...
    uv run python -m generate_cd \
        --input_file /gscratch/zlab/jyyh/copy-bench/data/factscore.jsonl \
        --mode "local_kl_acp_fuse" \
        --clean_model_path $CLEAN_MODEL_PATH \
        --dirty_model_path meta-llama/Meta-Llama-3.1-${SIZE} \
        --output_file /gscratch/kohlab/jyyh/fixed_bs/factscore_${SIZE}_results_toklevel_temp_${TEMP}_k_radius_${K_RADIUS}_shard${size_idx}.jsonl \
        --batch_size ${BATCH_SIZE} \
        --num_workers 1 \
        --num_shards ${NUM_SHARDS} \
        --shard_id ${size_idx} \
...
# 80 sec / 2 samples for 7b, 8b pair 
